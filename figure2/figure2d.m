...
%% ========================================================================
% SCI-style 3D surf affinity landscape plots
% ========================================================================

%% ========================================================================
% SCI-style 3D surf affinity landscape plots using absolute abundance
% ========================================================================

t_grid = 0:2:2000;
dist = compute_affinity_distribution(t, y, t_grid);

% -------------------------------------------------------------------------
% Use absolute abundance directly, no log transformation
% -------------------------------------------------------------------------
IgG_plot = dist.kd_IgG';
IgM_plot = dist.kd_IgM';

% Shared color range for direct comparison between IgG and IgM
clim_shared = [ ...
    min([IgG_plot(:); IgM_plot(:)]), ...
    max([IgG_plot(:); IgM_plot(:)]) ...
];

% Avoid degenerate color limits
if clim_shared(1) == clim_shared(2)
    clim_shared = clim_shared + [-1 1];
end

% -------------------------------------------------------------------------
% SCI / Nature-like colormap
% -------------------------------------------------------------------------
sci_cmap = interp1([1 80 160 256], ...
    [0.96 0.97 0.98; ...   % near white
     0.58 0.74 0.86; ...   % soft blue
     0.98 0.70 0.42; ...   % orange
     0.65 0.08 0.10], ...  % deep red
    1:256);
...
%% ------------------------------------------------------------------------
% Panel a: IgG 3D surf, absolute abundance
% -------------------------------------------------------------------------
ax1 = nexttile(tl, 1);

surf(ax1, dist.kd_values, t_grid, IgG_plot, ...
    'EdgeColor', 'none', ...
    'FaceColor', 'interp', ...
    'FaceAlpha', 0.98);
...
ylabel(ax1, 'Time, model units', ...
    'FontSize', 9.5, ...
    'FontWeight', 'bold');

zlabel(ax1, 'IgG abundance', ...
    'FontSize', 9.5, ...
    'FontWeight', 'bold');

title(ax1, 'IgG abundance landscape', ...
    'FontSize', 10.5, ...
    'FontWeight', 'bold');

xticks(ax1, -31:3:-13);
yticks(ax1, 0:500:2000);

% Use scientific notation for absolute abundance axis
...
text(ax1, -0.16, 1.04, 1.02, 'a', ...
    'Units', 'normalized', ...
    'FontSize', 13, ...
    'FontWeight', 'bold');

%% ------------------------------------------------------------------------
% Panel b: IgM 3D surf, absolute abundance
% -------------------------------------------------------------------------
ax2 = nexttile(tl, 2);

surf(ax2, dist.kd_values, t_grid, IgM_plot, ...
    'EdgeColor', 'none', ...
    'FaceColor', 'interp', ...
    'FaceAlpha', 0.98);
...
ylabel(ax2, 'Time, model units', ...
    'FontSize', 9.5, ...
    'FontWeight', 'bold');

zlabel(ax2, 'IgM abundance', ...
    'FontSize', 9.5, ...
    'FontWeight', 'bold');

title(ax2, 'IgM abundance landscape', ...
    'FontSize', 10.5, ...
    'FontWeight', 'bold');

xticks(ax2, -31:3:-13);
yticks(ax2, 0:500:2000);
...
text(ax2, -0.16, 1.04, 1.02, 'b', ...
    'Units', 'normalized', ...
    'FontSize', 13, ...
    'FontWeight', 'bold');

%% ------------------------------------------------------------------------
% Shared colorbar using absolute abundance
% -------------------------------------------------------------------------
cb = colorbar(ax2);
cb.Layout.Tile = 'east';
...
% Scientific notation on colorbar ticks
cb.Ruler.Exponent = 0;
cb.TickLabels = arrayfun(@(x) sprintf('%.1e', x), cb.Ticks, 'UniformOutput', false);

sgtitle(tl, ...
    'Affinity-resolved IgM and IgG abundance dynamics', ...
    'FontName', 'Arial', ...
    'FontSize', 12, ...
    'FontWeight', 'bold');
...
% -------------------------------------------------------------------------
exportgraphics(fig, 'affinity_landscape_IgG_IgM_3D_surf_absolute_SCI.png', ...
    'Resolution', 600, ...
    'BackgroundColor', 'white');

exportgraphics(fig, 'affinity_landscape_IgG_IgM_3D_surf_absolute_SCI.pdf', ...
    'ContentType', 'vector', ...
    'BackgroundColor', 'white');