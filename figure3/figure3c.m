...
I0 = zeros(p.N, 1);      % 初始感染细胞 (各龄均为0)
V0 = 10;                % 初始胞外病毒
Tc0 = interp1(t,y(:,403),1000);               % 初始 Tc 细胞 1e5
...
for i = 1:1800
     ...
     x0(i) = interp1(t,y(:,i+403),1000);

end

for i = 1801:3600
     x0(i) = 1.0*interp1(t,y(:,i+403),0)+0.0*interp1(t,y(:,i+403),1000);
end
...
%% ========================================================================
% Optional summary: masking fraction and infection-active fraction
% ========================================================================

V_second = y1(:, p.N + 2);                 % extracellular free virus

X3603_RBD_complex = y1(:, p.N + 3 + 3603);     % X(3603), V-A1 complex, RBD epitope
X3604_nonRBD_complex = y1(:, p.N + 3 + 3604);  % X(3604), V-A2 complex, non-RBD epitope

% Epitope-associated viral particle numbers
RBD_exposed_active = V_second + X3603_RBD_complex;
nonRBD_total_masked = V_second + X3604_nonRBD_complex;

% Avoid tiny negative numerical artifacts
V_second = max(V_second, 0);
X3603_RBD_complex = max(X3603_RBD_complex, 0);
X3604_nonRBD_complex = max(X3604_nonRBD_complex, 0);
RBD_exposed_active = max(RBD_exposed_active, 0);
nonRBD_total_masked = max(nonRBD_total_masked, 0);
...
% Color palette
col_V      = [0.20 0.20 0.20];   % dark gray
col_RBD    = [0.10 0.35 0.80];   % blue
col_nonRBD = [0.80 0.22 0.15];   % red-orange
col_active = [0.00 0.55 0.35];   % green
col_masked = [0.55 0.15 0.65];   % purple
...
plot(ax, t1, X3603_RBD_complex, '--', ...
    'Color', col_RBD, ...
    'LineWidth', 2.0);

plot(ax, t1, X3604_nonRBD_complex, '--', ...
    'Color', col_nonRBD, ...
    'LineWidth', 2.0);

plot(ax, t1, RBD_exposed_active, '-', ...
    'Color', col_active, ...
    'LineWidth', 2.6);

plot(ax, t1, nonRBD_total_masked, '-', ...
    'Color', col_masked, ...
    'LineWidth', 2.6);
...
xlabel(ax, 'Time after secondary infection', ...
    'FontSize', 10.5, ...
    'FontWeight', 'bold');

ylabel(ax, 'Viral particle / epitope-associated particle number', ...
    'FontSize', 10.5, ...
    'FontWeight', 'bold');

title(ax, {'Epitope masking mechanism', ...
           'RBD-mutated secondary infection'}, ...
    'FontSize', 12, ...
    'FontWeight', 'bold');
...
legend(ax, ...
    {'Free virus, V', ...
     'RBD complex, X(3603)', ...
     'non-RBD complex, X(3604)', ...
     'RBD exposed / infection-active, V + X(3603)', ...
     'non-RBD occupied / masked, V + X(3604)'}, ...
    'Location', 'northeast', ...
    'Box', 'off', ...
    'FontSize', 8.5);
...
% If values span several orders of magnitude, log scale is clearer
set(ax, 'YScale', 'log');

% Prevent log-scale problems if all values are very small
all_vals = [V_second; X3603_RBD_complex; X3604_nonRBD_complex; ...
            RBD_exposed_active; nonRBD_total_masked];
positive_vals = all_vals(all_vals > 0);

if ~isempty(positive_vals)
    ylim(ax, [max(min(positive_vals) * 0.5, 1e-12), max(positive_vals) * 2]);
end
...
text(ax, xl(1) + 0.05 * range(xl), yl(2) / 3, ...
    {'Mutated RBD remains exposed', ...
     'available for BCR recognition'}, ...
    'Color', col_active, ...
    'FontName', 'Arial', ...
    'FontSize', 8.5, ...
    'FontWeight', 'bold');

text(ax, xl(1) + 0.50 * range(xl), yl(2) / 12, ...
    {'Conserved non-RBD epitopes', ...
     'occupied by pre-existing antibodies'}, ...
    'Color', col_masked, ...
    'FontName', 'Arial', ...
    'FontSize', 8.5, ...
    'FontWeight', 'bold');
